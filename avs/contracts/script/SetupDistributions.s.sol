...
contract SetupDistributions is Script, Test {
    ...
    function setUp() public {
        ...
        feeShareDeployment =
            FeeShareDeploymentLib.readDeploymentJson("deployments/fee-share/", block.chainid);
        feeShareConfig =
    ...
    function run() external {
        vm.startBroadcast(feeShareConfig.rewardsInitiatorKey);
    ...
    function executeProcessClaim() public {
        ...
        vm.startBroadcast(deployer);
        earnerLeaves =
            _getEarnerLeaves(_getEarners(deployer), amountPerPayment, feeShareDeployment.strategy);
        processClaim(
    ...
    function createAVSRewardsSubmissions(
        ...
        ERC20Mock(feeShareDeployment.token).mint(
            feeShareConfig.rewardsInitiator, _amountPerPayment * _numPayments
        );
        ERC20Mock(feeShareDeployment.token).increaseAllowance(
            feeShareDeployment.feeShareServiceManager, _amountPerPayment * _numPayments
        );
